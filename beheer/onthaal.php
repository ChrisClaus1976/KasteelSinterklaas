<?php
	...
	<head>
		...
		<script type="text/javascript">	
			...
			function reservatie_tonen(reservatieid)
			{
				...
				xHTTP.open("GET", "ajax/onthaalreservatie.php?reservatieid=" + reservatieid, true);
				xHTTP.send();
			}
			
			
			function WijzigAanwezigKind(reservatieid, veld, waarde)
			{
				...
				xHTTP.open("GET", "ajax/onthaalwijzigaanwezigkind.php?reservatieid=" + reservatieid + "&veld=" + veld + "&waarde=" + waarde, true);
				xHTTP.send();
			}
			
			function reservatie_aanwezig(reservatieid, waarde)
			{
	...
	if ($datum != '')
	{
		$query = $query. " WHERE TBL_data.Datum='$datum'";
	}
	$query = $query. " ORDER BY TBL_Reservatie.ReservatieAanwezig, TBL_data.Datum, TBL_data.Tijdstip ";
	...
	if ($aantalrecords>0)
	{
		...
		while($row = mysqli_fetch_assoc($result))
		{
			...
			if ($Canceled!=1)
			{
				echo "<input type=\"button\" class=\"buttongroot\" onclick=\"reservatie_tonen('$ReservatieId')\" value=\"$Tijdstip - $Naam ($AantalVolwassenen-$AantalKinderen) - $ReservatieId\" style=\"height: 50px; width: 300px;\"><br><br>";
			}
		}
		//echo "</table>";
	}
